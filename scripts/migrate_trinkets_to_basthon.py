"""One-off migration: replace live Trinkets with Basthon and remove only Parsons sections."""
...
def safe_name(name: str) -> str | None:
    ...
    path = Path(name)
    return None if not name or ".." in path.parts else name
...
def snapshot(kind: str, trinket_id: str) -> dict:
    ...
    if main_name.endswith(".py"):
        try:
            compile((directory / main_name).read_text(encoding="utf-8"), str(directory / main_name), "exec")
    ...
    if assets:
        report["warnings"].append(f"Trinket {trinket_id} has {len(assets)} separate asset(s); inspect if used.")
...
def remove_parsons(text: str) -> tuple[str, int]:
    ...
    # Remove a Parsons heading and only its section (to the next heading of the
    # same or higher level), never the whole Markdown cell.
    for i, line in enumerate(lines):
        heading = HEADING_RE.match(line.strip("\n"))
        if not heading or "parsons" not in heading.group(2).lower():
            continue
        level = len(heading.group(1))
        end = len(lines)
        for j in range(i + 1, len(lines)):
            next_heading = HEADING_RE.match(lines[j].strip("\n"))
    ...
    cleaned = "".join(line for i, line in enumerate(lines) if i not in remove)
...
def validate(files: list[Path]) -> None:
    ...
    for path in files:
        ...
        if "parsons" in text:
            errors.append(f"Parsons content remains in {path.relative_to(ROOT)}")
...
def main() -> None:
    ...
    print(f"Removed {total_parsons} Parsons sections/blocks/references.")
